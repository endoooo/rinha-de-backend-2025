defmodule PaymentProcessor.PaymentRouter do
  # alias PaymentProcessor.ProcessorMonitor
  alias PaymentProcessor.ProcessorClient

  def route_payment(correlation_id, amount, requested_at) do
    attempt_payment(:default, correlation_id, amount, requested_at)
  ...
  defp attempt_payment(processor_type, correlation_id, amount, requested_at) do
    case ProcessorClient.process_payment(processor_type, correlation_id, amount, requested_at) do
      {:ok, response} ->
        {:ok, processor_type, response}

      {:error, _reason} = error ->
        if processor_type == :default do
          attempt_payment(:fallback, correlation_id, amount, requested_at)
        else
          error
        end
    end
